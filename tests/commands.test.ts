...
test("rescue command absorbs continue semantics", () => {
  ...
  assert.match(rescue, /allowed-tools:\s*Bash\(bun:\*\),\s*AskUserQuestion,\s*Agent/);
  ...
  assert.match(rescue, /default the Codex task to background/i);
  assert.match(rescue, /forward `--background` to `task`/i);
  ...
  assert.match(rescue, /Companion script path:/i);
  assert.match(rescue, /`\$\{CLAUDE_PLUGIN_ROOT\}\/scripts\/codex-companion\.ts`/);
  assert.match(rescue, /Leave `--resume` and `--fresh` in the forwarded request/i);
  assert.match(agent, /--resume/);
  assert.match(agent, /--fresh/);
  assert.match(agent, /thin forwarding wrapper/i);
  assert.match(agent, /Use the absolute path supplied in the `Companion script path` field/i);
  ...
  assert.match(runtimeSkill, /use the `codex-prompting` skill to tighten the user's request before forwarding it/i);
...
test("internal docs use task terminology for rescue runs", () => {
  ...
  assert.match(promptingSkill, /lean, outcome-oriented prompt/i);
  assert.match(promptingSkill, /bounded parallel lanes/i);
  assert.match(promptingSkill, /reproduce or verify accepted child-agent findings/i);
  assert.match(promptRecipes, /Codex task prompts/i);
  assert.match(promptRecipes, /compact starting points for Codex task prompts/i);
...
test("setup command can offer Codex install and still points users to codex login", () => {
  ...
  assert.match(setup, /bun add --global @openai\/codex/);
